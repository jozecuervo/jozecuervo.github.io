...
*How brain surgery, staff engineering, and AI agents led me to build a reviewable cognitive prosthetic.*
...
The code was not the hard part. The hard part was everything around it: meeting notes, Slack threads, Jira tickets, half-decided plans, pull requests waiting on humans, org changes, old decisions that still mattered, new decisions that needed evidence, and the re-entry problem of asking "where was I?" after a day of interruptions.
...
There is a tempting way to describe this system: "AI that never forgets." That is too generous and technically false. Agents forget. Notes rot. Sources go stale. Old claims get laundered through summaries. A transcript on disk is not memory if nobody can retrieve it at the right time.
...
That is what I accidentally built for myself. When my doctor saw it, he said: you should build this for people with traumatic brain injuries.

## The System Is Three Kinds of Memory

In 2026, the agent-memory world has mostly converged on a useful taxonomy. My workspace already had all three types, just under ordinary filenames:

| Memory type | In the workspace | Purpose |
|---|---|---|
| Episodic | standups, ingest ledgers, journey logs, transcripts | Preserves what happened, in order, including wrong turns. |
| Semantic | research dossiers, citation indexes, registries, project maps, current-state files | Distills what we believe is true, with source, method, date observed, refresh-by date, caveats, and reviewer. |
| Procedural | `AGENTS.md`, `MAP.md`, `CONFIG.md`, `NOTES.md`, templates, scripts, skills | Tells agents how to behave, where to look, what not to touch, and when to ask first. |
...
In this system, the boundaries stay explicit:
...
The archive rule mattered more than I expected. If a decision today resembles an old one, I either follow the old decision or explain why this case is different. I stole that from legal reasoning. It turns out to be exactly what an external memory system needs: not just recall, but continuity of judgment.
...
Two decades later, I accidentally rebuilt a tiny, file-backed version for myself. Mine is smaller and more personal than Engelbart's societal vision, but the shape is familiar: capture the swirl of work, preserve rationale, connect evidence to decisions, keep the repository alive, and improve the human and tool systems together.

The timing matters because this is not nostalgia. Engelbart's DKR idea is still live. In 2024, Fraga, Poggi, Casanova, and Leme published work on automatic connections for personal knowledge management, using NLP and knowledge graphs to connect isolated text collections. The vocabulary has changed: PKM, knowledge graphs, semantic similarity, LLMs. The problem is recognizably the same.

I do not want to overstate the anecdote. Having lunch with Engelbart did not make me his intellectual heir. But it adds an odd continuity. The ideas were in the room with me in 2004. I had to live long enough, get injured enough, and build enough systems to understand why they mattered.
...
That was not a polished metaphor. It was a literal description of re-entry.

Comments I had written years earlier bootstrapped me back into systems I had once understood. The code held context while my brain recovered. It reduced the load of remembering how to think about the work.

The staff-engineer workspace generalized that idea beyond code.

What if meetings had the same property? What if a scattered Slack thread became a dated note with decisions, owners, open questions, and links? What if every project had a current-state file that told future me where to re-enter? What if the system checked Jira and GitHub and told me when the world had drifted from my notes? What if a bad cognitive day did not mean the thread was dropped?

I thought I was building a better professional workflow. True, but incomplete.
...
The clinical point is not that apps cure brain injury. They do not. The narrower, more useful point is that well-designed external supports can improve real-world function, especially when memory and executive function are impaired.

That was the first bridge to my system. The second was more specific: the gap is not reminders. The gap is coordination.

## The Missing Layer Is Connective Tissue

A reminder is useful. It is not enough.

People with brain injuries do not only need to remember one appointment. They need to coordinate providers, medications, insurance, caregivers, forms, fatigue, family communication, and work. They need continuity on days when cognition fluctuates. They need handoffs that do not require retelling the same story from scratch. They need a system that assumes the skills required to manage care - planning, sequencing, monitoring, remembering future intentions - may be the skills that were injured.
...
That is still true, but the stronger evidence now is architectural.

Across agent systems, the frame has shifted from "retrieval" to "write path." A RAG system can read from a knowledge base. A memory system has to update state: notice change, link new facts to old ones, mark claims stale, and preserve enough provenance that a human can inspect the chain later.

That is exactly what a cognitive support system needs.

If a provider changes medication instructions, the system cannot just answer questions about the old plan. It needs to update the plan, preserve the old instruction as superseded, record where the new instruction came from, and surface downstream tasks. If an appointment is cancelled, the calendar event is not enough. The open loop is the reschedule, the transportation change, the caregiver notification, and the "do we still have enough medication until then?" question.

That is a write-path problem.

The same is true in my work system. If a Jira ticket closes, the registry row changes, but that is not enough. The lane README may need an update, the TODO may need a status change, a stakeholder note may need a different commitment, and a pitch may need its evidence chain refreshed.
...
- a human-readable filesystem so the operator can inspect and repair it.

That is why the file-backed version matters. A proprietary black-box memory could be impressive, but it would fail the trust test. If I cannot inspect the memory, correct it, cite it, and see what changed, it is not a prosthetic. It is another system I have to supervise.

## The Design Principle: Drafts, Never Acts

For engineering work, I already had a rule: agents draft, humans decide.

That sounds like a safety rule. It is also a disability-design principle.

A cognitive prosthetic should not seize agency from the person using it. It should reduce the cost of agency: hold the thread, prepare the options, surface the mismatch, and make the next decision easier. The person remains the author of durable truth.

That matters clinically because the goal is not to replace a damaged human with a compliant machine. The goal is to change the environment so the person can function with the brain they have.
...
I do not think the first version should be a general chatbot. It should be an executable care notebook.
...
The loop would turn ordinary mess into reviewable state:

| Stage | Job |
|---|---|
| Inputs | Accept voice memos, discharge photos, portal messages, medication changes, caregiver texts, therapy homework, insurance denials, and calendar invites. |
| Ingest | Record what happened, who said it, what changed, what needs to happen next, who owns it, what date matters, what source supports it, and what remains uncertain. |
| Daily briefing | Provide a re-entry surface: today, due soon, waiting on, changed since last check, risks, and one or two next actions. |
| Drift check | Compare planned state with observed state: medication expected but not logged, appointment expected but not confirmed, referral promised but not received, bill due but no payment record, caregiver question unanswered, provider handoff stale, or claim past its refresh date. |
| Handoff | Generate a concise, source-linked summary for a new provider, therapist, caregiver, attorney, school, employer, or family member. |

The system would not have to be glamorous to matter. NeuroPage was a pager. Its power was not interface polish; it was that the cue arrived when the person needed it.

## The Hard Parts

The hard parts are not the demo. They are consent, privacy, clinical fit, caregiver boundaries, failure modes, and avoiding false confidence.

A system like this cannot silently rewrite medical history because an LLM summarized too aggressively. It cannot invent source confidence, collapse patient voice into caregiver convenience, turn every nudge into surveillance, or require the user to become a systems administrator on a bad cognitive day.
...
- a strong bias toward "prepare the decision" rather than "make the decision."
...
But the architecture matters more than the autobiography. If this is only my story, it is moving but not necessarily reusable. If it is an architecture, it can be tested.
...
That is the path that feels real to me now: build a reviewable prototype with occupational therapists, neuropsychologists, patients, and caregivers; test whether the loop reduces real coordination failures; and keep the human review gate central.
...
I still think that is the right image, but I understand it differently now.
...
That is what this system is trying to do. Not remember everything. Not automate the person. Not turn a life into a database. Preserve the thread.

For staff engineering, that means decisions, tickets, PRs, and cross-team commitments. For brain injury recovery, it could mean appointments, medications, instructions, symptoms, forms, and family communication. Different domain. Same shape.
